    ...
    while condition < 200:
        ...
        while elapsed_steps < max_steps:
            ...
            if done:
                ...
                if info["success"]:
                   success += 1
                   success_rate = success / instances
                   print(f"{condition}回目でSuccess rate1: {success_rate:.3f}")
                break
...
class TimeLimitWrapper(_TimeLimit):
    def __init__(self, env, bonus_steps: int = 0):
        self.max_steps = env.task.oracle_max_steps + bonus_steps
        super().__init__(env, self.max_steps)
